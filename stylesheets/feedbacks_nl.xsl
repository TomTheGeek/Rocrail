...
<xsl:template match="/">
   ...
   <html>
   <head><title>Rocrail Terugmelders</title>
  </head>
  <body>
  <h2>Terugmelders</h2>
  ...
  <table border="1" rules="all" cellpadding="4" cellspacing="0" width="80%">
	  ...
	  <TR>
	     <TH width="20%" height="30" bgcolor="lightgrey">ID</TH>
 	     <TH width="40%" bgcolor="lightgrey">Omschrijving</TH>
 	     <TH width="20%" bgcolor="lightgrey">IID</TH>
 	     <TH width="20%" bgcolor="lightgrey">Adres</TH>
	  </TR>
      </THEAD> 
      <TBODY>
        <xsl:apply-templates/>
    ...
    <xsl:for-each select="../../bklist/bk">
          <xsl:variable name="bkid" select="@id"/>
            <xsl:for-each select="fbevent">
			  ...
			  <tr>
			    <td></td>
                <td colspan="3"><strong>Blok: </strong> <xsl:value-of select="$bkid"/> -&gt; <xsl:value-of select="@action"/>
...
<xsl:template match="fbaction">  
	...
	<TR>	
	   <TD></TD>
	   <TD colspan="3">
			<strong>Actie: </strong><xsl:value-of select="@type" /> -&gt; <xsl:value-of select="@id" /> -&gt; <xsl:value-of select="@cmd" />	
		</TD>	
	</TR>	   
