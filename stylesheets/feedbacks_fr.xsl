...
<xsl:template match="/">
  ...
  <body>
  <h2>Rapport des modules de r&#233;trosignalisation</h2>
  ...
  <table border="1" rules="all" cellpadding="4" cellspacing="0" width="80%">
	  ...
	  <TR>
	     <TH width="20%" height="30" bgcolor="lightgrey">ID</TH>
 	     ...
 	     <TH width="20%" bgcolor="lightgrey">Adresse</TH>
	  </TR>
      </THEAD> 
      <TBODY>
        <xsl:apply-templates/>
    ...
    <xsl:for-each select="../../bklist/bk">
          <xsl:variable name="bkid" select="@id"/>
            <xsl:for-each select="fbevent">
			  ...
			  <tr>
			    <td></td>
                <td colspan="3"><strong>Bloc: </strong> <xsl:value-of select="$bkid"/> -&gt; <xsl:value-of select="@action"/>
				de Bloc <xsl:value-of select="@from"/>
				</td>
			  </tr>	
              </xsl:if>
            </xsl:for-each>
