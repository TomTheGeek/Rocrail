    ...
    <xsl:for-each select="../../bklist/bk">
          <xsl:variable name="bkid" select="@id"/>
            <xsl:for-each select="fbevent">
			  ...
			  <tr>
			    <td></td>
                <td colspan="3"><strong>Blocco: </strong> <xsl:value-of select="$bkid"/> -&gt; <xsl:value-of select="@action"/>
				da blocco <xsl:value-of select="@from"/>
				</td>
			  </tr>	
              </xsl:if>
            </xsl:for-each>
...
<xsl:template match="fbaction">  
	...
	<TR>	
	   <TD></TD>
	   <TD colspan="3">
			<strong>Aziona: </strong><xsl:value-of select="@type" /> -&gt; <xsl:value-of select="@id" /> -&gt; <xsl:value-of select="@cmd" />	
		</TD>	
	</TR>	   
