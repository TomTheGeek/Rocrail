...
static void __broadcastLocoProps( iOLoc inst, const char* cmd ) {
  iOLocData data = Data(inst);
...
static void* __event( void* inst, const void* evt ) {
  ...
  if( StrOp.equals( wLoc.name(), NodeOp.getName(evtNode) ) && StrOp.equals( wLoc.bidikmh, wLoc.getcmd(evtNode) ) ) {
    ...
    __broadcastLocoProps(inst, wLoc.bidikmh);
    return NULL;
  }
  ...
  if( StrOp.equals( wLoc.name(), NodeOp.getName(evtNode) )) {
    ...
    if( broadcast ) {
      __broadcastLocoProps(inst, NULL);
    }
...
*/
static Boolean __engine( iOLoc inst, iONode cmd, Boolean blocking ) {
  iOLocData    data = Data(inst);
  ...
  if( blocking && !MutexOp.trywait( data->muxEngine, 1000 ) ) {
...
static Boolean _cmd( iOLoc inst, iONode nodeA ) {
  ...
  if( !__engine( inst, nodeA, data->blocking ) ) {
