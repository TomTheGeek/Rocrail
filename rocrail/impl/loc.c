...
static void* __event( void* inst, const void* evt ) {
  ...
  if( StrOp.equals( wLoc.name(), NodeOp.getName(evtNode) )) {
    ...
    data->fn4 = wFunCmd.isf4(evtNode);

    TraceOp.trc( name, TRCLEVEL_INFO, __LINE__, 9999, "lc=%s f0=%s f1=%s f2=%s f3=%s f4=%s",
        wLoc.getid( data->props ), wLoc.isfn(data->props) ? "on":"off", data->fn1 ? "on":"off", data->fn2 ? "on":"off", data->fn3 ? "on":"off", data->fn4 ? "on":"off" );
    /* Broadcast to clients. */
    {
...
static obj __smoothEvents( iOLoc loc, obj emitter, int * event ) {
    ...
    switch( *event ) {
      case enter_event:
        if( !data->enter ) {
          data->enter = True;
          data->in = False;
          data->enterEmitter = emitter;
          data->enterId = blockId;
          data->enterTick = SystemOp.getTick();
          *event = enter_event;
          return emitter;
        }
        else if( StrOp.equals( blockId, data->enterId ) ) {
          data->in = False;
          data->enterTick = SystemOp.getTick();
          *event = -1;
          return emitter;
      ...
      case exit_event:
        if( !data->exit ) {
          data->exit = True;
          data->out = False;
          data->exitEmitter = emitter;
          data->exitId = blockId;
          data->exitTick = SystemOp.getTick();
          *event = exit_event;
          return emitter;
        }
        else if( StrOp.equals( blockId, data->exitId ) ) {
          data->out = False;
          data->exitTick = SystemOp.getTick();
          *event = -1;
          return emitter;
  ...
  if( data->enter && data->in ) {
    if( SystemOp.getTick() - data->enterTick > data->ignevt ) {
      data->enter = False;
      data->in = False;
      emitter = data->enterEmitter;
      *event = in_event;
    }
  }
