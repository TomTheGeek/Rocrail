...
int raspiWriteRegI2C(int descriptor, unsigned char dev_addr, unsigned char reg_addr, unsigned char data) {
  ...
  packets.nmsgs = 1;
  TraceOp.trc( "raspi-i2c", TRCLEVEL_INFO, __LINE__, 9999, "write I2C register: addr=0x%02X reg=0x%02X msg=0x%02X", dev_addr, reg_addr, data );
...
int raspiReadRegI2C(int descriptor, unsigned char dev_addr, unsigned char reg_addr, unsigned char* data) {
  ...
  packets.msgs = messages;
  packets.nmsgs = 2;
  TraceOp.trc( "raspi-i2c", TRCLEVEL_INFO, __LINE__, 9999, "read I2C register: addr=0x%02X reg=0x%02X", dev_addr, reg_addr );
  return ioctl(descriptor, I2C_RDWR, &packets);
}
