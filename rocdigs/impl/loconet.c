...
static void __handleSensor(iOLocoNet loconet, int addr, int value, Boolean Lissy) {
  iOLocoNetData data = Data(loconet);

  TraceOp.trc( name, TRCLEVEL_INFO, __LINE__, 9999, "fb %d=%d (%s)", addr, value, Lissy?"Lissy":"" );
  {
...
static void __evaluatePacket(iOLocoNet loconet, byte* rsp, int size ) {
  ...
  case OPC_INPUT_REP: // B2
    ...
    __handleSensor(loconet, addr, value, False);
    break;
  ...
  case OPC_LISSY_REP: // E4
    ...
    __handleSensor(loconet, addr, value, True);
    break;
  ...
  case OPC_PEER_XFER: // E5
  {
    ...
    if( rsp[1] == 0x0F && rsp[2] == UB_SRC_MASTER && rsp[3] == 0x49 && rsp[4] == 0x4B ) {
      ...
      for( saddr = 0; saddr < 8; saddr++ ) {
        value = d6 & (0x01 << saddr);
        if( value )
          __handleSensor(loconet, addr+(7-saddr)+1, 1, False);
      }
  ...
  /* Loc command. */
    ...
    if( MutexOp.trywait( data->slotmux, 1000 ) ) {
      ...
      if( slot > 0 && StrOp.equals( wLoc.dispatch, wLoc.getcmd(node) ) ) {
        ...
        data->locoslot[slot] = 0;
        data->slotaccessed[slot] = 0;
        size = makereqDispatch(cmd, slot, wLoc.getaddr(node), status );
      }
      else if( slot > 0 ) {
 ...
 */
int makereqDispatch(byte *msg, int slot, int addr, int status) {
