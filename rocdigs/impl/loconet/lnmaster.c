...
static iONode __locCmd(iOLocoNet loconet, int slotnr, struct __lnslot* slot);
      ...
      for( i = 1; i < 120; i++ ) {
        if( slot[i].inuse && ((currtime - slot[i].accessed) > (data->purgetime)) ) {
          ...
          if( wLNSlotServer.isstopatpurge(data->slotserver) ) {
            ...
            data->listenerFun( data->listenerObj, __locCmd( loconet, i, slot), TRCLEVEL_INFO );
          }
        }
      }
    }
...
static iONode __locCmd(iOLocoNet loconet, int slotnr, struct __lnslot* slot) {
  ...
  TraceOp.trc( name, TRCLEVEL_INFO, __LINE__, 9999,
      "slot# %d format=%d steps=%d dir=%s inuse=%d",
      slotnr, slot[slotnr].format, slot[slotnr].steps, slot[slotnr].dir?"fwd":"rev", slot[slotnr].inuse );
